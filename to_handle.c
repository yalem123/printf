 ...
 * @index: string index counter.
 * Return: If flag char are matched - a corresponding value.
 * else - 0.
 */
unsigned char handle_flags(const char *flag, char *index)
{
 ...
 * @specifier: A pointer to conv specifier.
 * Return: If a conv function is matched - a pointer to the function.
 * else - NULL.
 */
unsigned int (*handle_specifiers(const char *specifier))(va_list, buffer_t *,
		unsigned char, int, int, unsigned char)
